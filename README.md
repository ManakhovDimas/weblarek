...
###### Header (Шапка)
Назначение: Отображение шапки с кнопкой корзины
Методы:
- `setCounter(count)` - обновление счетчика товаров

###### GalleryView (Галерея товаров)
Назначение: Отображение каталога товаров
Методы:
- `setItems(items)` - установка элементов каталога

###### BasketView (Корзина)
Назначение: Отображение корзины покупок
Методы:
- `setItems(items)` - установка элементов корзины
- `setTotalPrice(price)` - установка общей суммы
- `setOrderButtonEnabled(enabled)` - активация кнопки заказа

###### OrderForm (Форма заказа)
Назначение: Форма выбора способа оплаты и адреса
Методы:
- `setPaymentMethod(method)` - установка способа оплаты
- `setAddress(address)` - установка адреса
- `setValidationErrors(errors)` - отображение ошибок валидации

###### ContactsForm (Форма контактов)
Назначение: Форма ввода контактных данных
Методы:
- `setEmail(email)` - установка email
- `setPhone(phone)` - установка телефона
- `setValidationErrors(errors)` - отображение ошибок валидации

###### ProductPreview (Превью товара)
Назначение: Детальное отображение товара
Методы:
- `updateProductData(data, isInCart)` - обновление данных товара
- `setButtonText(text)` - изменение текста кнопки
- `setButtonDisabled(disabled)` - активация/деактивация кнопки
...
Свойства:
- `container` - DOM-элемент компонента
Методы:
...
Методы:
- `setTitle(title)` - установка заголовка
- `setPrice(price)` - установка цены

###### Form
Назначение: Базовый класс для форм
Методы:
- `setErrors(errors)` - отображение ошибок
- `setSubmitButtonEnabled(enabled)` - активация кнопки отправки
- `setupSubmitHandler(callback)` - настройка обработчика отправки

###### Поток данных

1. Инициализация приложения

App.init() → loadCatalog() → API запрос → CATALOG_LOADED → renderCatalog()


2. Выбор товара

Пользователь кликает на товар → CARD_CLICK → PRODUCT_SELECTED → открытие модального окна с превью


3. Добавление в корзину

Клик "В корзину" → PREVIEW_TOGGLE → CART_UPDATED → обновление счетчика в Header


 4. Оформление заказа

Открытие корзины → ORDER_OPEN → заполнение формы заказа → ORDER_SUBMIT → 
→ CONTACTS_OPEN → заполнение контактов → CONTACTS_SUBMIT → submitOrder() → 
→ API запрос → показ SuccessView
